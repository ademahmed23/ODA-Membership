...
final class Zone4Table extends PowerGridComponent
{
    use ActionButton;
    ...
    */
     ...
     *
     * @return Builder<\App\Models\Zone4>
     */
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('first_name_lower', function (Zone4 $model) {
    ...
    */
    ...
    /**
     * PowerGrid Zone4 Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::make('edit', 'Edit')
                ...
                ->target('_self')
                ->route('zone4.edit', ['zone4' => 'id']),

            Button::make('destroy', 'Delete')
                ->class('bx bx-trash me-1')
                ->target('_self')
                ->route('zone4.destroy', ['zone4' => 'id'])
                ->method('delete'),
    ...
    */
    ...
    public function actionRules(): array
    {
        return [
            ...
            //->hide(),
            Rule::button('edit')
                ->when(fn () => Auth::user()->cannot('zone4-edit'))
                ->hide(),

            Rule::button('delete')
                ->when(fn () => Auth::user()->cannot('zone4-delete'))
                ->hide(),
            Rule::button('pay')
                ->when(fn ($zone4) => ZoneMemberPay::where('member_id', $zone4->id)->where('model', 'zone4')->whereMonth('date', now()->month)->whereYear('date', now()->year)->exists())
                ->hide(),
            Rule::button('paid')
                ->when(fn ($zone4) => !ZoneMemberPay::where('member_id', $zone4->id)->where(
                    'model',
                    'zone4'
                )->whereMonth(
                    'date',
