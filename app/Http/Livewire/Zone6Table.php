...
final class Zone6Table extends PowerGridComponent
{
    use ActionButton;
    ...
    */
     ...
     *
     * @return Builder<\App\Models\Zone6>
     */
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('first_name_lower', function (Zone6 $model) {
...
Column::make('Member Id', 'member_id')
    ...
    */
    ...
    /**
     * PowerGrid Zone6 Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::make('edit', 'Edit')
                ...
                ->target('_self')
                ->route('zone6.edit', ['zone6' => 'id']),

            Button::make('destroy', 'Delete')
                ->class('bx bx-trash me-1')
                ->target('_self')
                ->route('zone6.destroy', ['zone6' => 'id'])
                ->method('delete'),
    ...
    */
    ...
    public function actionRules(): array
    {
        return [
            ...
            //->hide(),
            Rule::button('edit')
                ->when(fn () => Auth::user()->cannot('zone6-edit'))
                ->hide(),

            Rule::button('delete')
                ->when(fn () => Auth::user()->cannot('zone6-delete'))
                ->hide(),
            Rule::button('pay')
                ->when(fn ($zone6) => ZoneMemberPay::where('member_id', $zone6->id)->where('model', 'zone6')->whereMonth('date', now()->month)->whereYear('date', now()->year)->exists())
                ->hide(),
            Rule::button('paid')
                ->when(fn ($zone6) => !ZoneMemberPay::where('member_id', $zone6->id)->where(
                    'model',
                    'zone6'
                )->whereMonth(
                    'date',
