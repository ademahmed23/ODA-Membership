...
final class Zone16Table extends PowerGridComponent
{
    use ActionButton;
    ...
    */
     ...
     *
     * @return Builder<\App\Models\Zone16>
     */
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('first_name_lower', function (Zone16 $model) {
    ...
    */
    ...
    /**
     * PowerGrid Zone16 Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::make('edit', 'Edit')
                ...
                ->target('_self')
                ->route('zone16.edit', ['zone16' => 'id']),

            Button::make('destroy', 'Delete')
                ->class('bx bx-trash me-1')
                ->target('_self')
                ->route('zone16.destroy', ['zone16' => 'id'])
                ->method('delete'),
    ...
    */
    ...
    public function actionRules(): array
    {
        return [
            ...
            //->hide(),
            Rule::button('edit')
                ->when(fn () => Auth::user()->cannot('zone16-edit'))
                ->hide(),

            Rule::button('delete')
                ->when(fn () => Auth::user()->cannot('zone16-delete'))
                ->hide(),
            Rule::button('pay')
                ->when(fn ($zone16) => ZoneMemberPay::where('member_id', $zone16->id)->where('model', 'zone16')->whereMonth('date', now()->month)->whereYear('date', now()->year)->exists())
                ->hide(),
            Rule::button('paid')
                ->when(fn ($zone16) => !ZoneMemberPay::where('member_id', $zone16->id)->where(
                    'model',
                    'zone16'
                )->whereMonth(
                    'date',
