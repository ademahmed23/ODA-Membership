...
final class Zone20Table extends PowerGridComponent
{
    use ActionButton;
    ...
    */
     ...
     *
     * @return Builder<\App\Models\Zone20>
     */
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('first_name_lower', function (Zone20 $model) {
    ...
    */
    ...
    /**
     * PowerGrid Zone20 Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::make('edit', 'Edit')
                ...
                ->target('_self')
                ->route('zone20.edit', ['zone20' => 'id']),

            Button::make('destroy', 'Delete')
                ->class('bx bx-trash me-1')
                ->target('_self')
                ->route('zone20.destroy', ['zone20' => 'id'])
                ->method('delete'),
    ...
    */
    ...
    public function actionRules(): array
    {
        return [
            ...
            //->hide(),
            Rule::button('edit')
                ->when(fn () => Auth::user()->cannot('zone20-edit'))
                ->hide(),

            Rule::button('delete')
                ->when(fn () => Auth::user()->cannot('zone20-delete'))
                ->hide(),
            Rule::button('pay')
                ->when(fn ($zone20) => ZoneMemberPay::where('member_id', $zone20->id)->where('model', 'zone20')->whereMonth('date', now()->month)->whereYear('date', now()->year)->exists())
                ->hide(),
            Rule::button('paid')
                ->when(fn ($zone20) => !ZoneMemberPay::where('member_id', $zone20->id)->where(
                    'model',
                    'zone20'
                )->whereMonth(
                    'date',
