...
final class Zone8Table extends PowerGridComponent
{
    use ActionButton;
    ...
    */
     ...
     *
     * @return Builder<\App\Models\Zone8>
     */
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('first_name_lower', function (Zone8 $model) {
    ...
    */
    ...
    /**
     * PowerGrid Zone8 Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::make('edit', 'Edit')
                ...
                ->target('_self')
                ->route('zone8.edit', ['zone8' => 'id']),

            Button::make('destroy', 'Delete')
                ->class('bx bx-trash me-1')
                ->target('_self')
                ->route('zone8.destroy', ['zone8' => 'id'])
                ->method('delete'),
    ...
    */
    ...
    public function actionRules(): array
    {
        return [
            ...
            //->hide(),
            Rule::button('edit')
                ->when(fn () => Auth::user()->cannot('zone8-edit'))
                ->hide(),

            Rule::button('delete')
                ->when(fn () => Auth::user()->cannot('zone8-delete'))
                ->hide(),
            Rule::button('pay')
                ->when(fn ($zone8) => ZoneMemberPay::where('member_id', $zone8->id)->where('model', 'zone8')->whereMonth('date', now()->month)->whereYear('date', now()->year)->exists())
                ->hide(),
            Rule::button('paid')
                ->when(fn ($zone8) => !ZoneMemberPay::where('member_id', $zone8->id)->where(
                    'model',
                    'zone8'
                )->whereMonth(
                    'date',
