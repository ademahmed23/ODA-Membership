...
final class Zone9Table extends PowerGridComponent
{
    use ActionButton;
    ...
    */
     ...
     *
     * @return Builder<\App\Models\Zone9>
     */
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('first_name_lower', function (Zone9 $model) {
    ...
    */
    ...
    /**
     * PowerGrid Zone9 Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::make('edit', 'Edit')
                ...
                ->target('_self')
                ->route('zone9.edit', ['zone9' => 'id']),

            Button::make('destroy', 'Delete')
                ->class('bx bx-trash me-1')
                ->target('_self')
                ->route('zone9.destroy', ['zone9' => 'id'])
                ->method('delete'),
    ...
    */
    ...
    public function actionRules(): array
    {
        return [
            ...
            //->hide(),
            Rule::button('edit')
                ->when(fn () => Auth::user()->cannot('zone9-edit'))
                ->hide(),

            Rule::button('delete')
                ->when(fn () => Auth::user()->cannot('zone9-delete'))
                ->hide(),
            Rule::button('pay')
                ->when(fn ($zone9) => ZoneMemberPay::where('member_id', $zone9->id)->where('model', 'zone9')->whereMonth('date', now()->month)->whereYear('date', now()->year)->exists())
                ->hide(),
            Rule::button('paid')
                ->when(fn ($zone9) => !ZoneMemberPay::where('member_id', $zone9->id)->where(
                    'model',
                    'zone9'
                )->whereMonth(
                    'date',
