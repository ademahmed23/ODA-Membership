...
final class Zone3Table extends PowerGridComponent
{
    use ActionButton;
    ...
    */
     ...
     *
     * @return Builder<\App\Models\Zone3>
     */
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('first_name_lower', function (Zone3 $model) {
    ...
    */
    ...
    /**
     * PowerGrid Zone3 Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::make('edit', 'Edit')
                ...
                ->target('_self')
                ->route('zone3.edit', ['zone3' => 'id']),

            Button::make('destroy', 'Delete')
                ->class('bx bx-trash me-1')
                ->target('_self')
                ->route('zone3.destroy', ['zone3' => 'id'])
                ->method('delete'),
    ...
    */
    ...
    public function actionRules(): array
    {
        return [
            ...
            //->hide(),
            Rule::button('edit')
                ->when(fn () => Auth::user()->cannot('zone3-edit'))
                ->hide(),

            Rule::button('delete')
                ->when(fn () => Auth::user()->cannot('zone3-delete'))
                ->hide(),
            Rule::button('pay')
                ->when(fn ($zone2) => ZoneMemberPay::where('member_id', $zone2->id)->where('model', 'zone2')->whereMonth('date', now()->month)->whereYear('date', now()->year)->exists())
                ->hide(),
            Rule::button('paid')
                ->when(fn ($zone3) => !ZoneMemberPay::where('member_id', $zone3->id)->where(
                    'model',
                    'zone3'
                )->whereMonth(
                    'date',
