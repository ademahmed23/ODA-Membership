...
final class Zone1Table extends PowerGridComponent
{
    use ActionButton;
    ...
    */
     ...
     *
     * @return Builder<\App\Models\Zone1>
     */
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('first_name_lower', function (Zone1 $model) {
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('AGE', 'age')
                ->sortable()
                ->searchable(),
            Column::make('EdLEVEL', 'education_level')
            ->sortable()
            ->searchable(),
    ...
    */
    ...
    /**
     * PowerGrid Zone1 Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::make('edit', 'Edit')
                ...
                ->target('_self')
                ->route('zone1.edit', ['zone1' => 'id']),

            Button::make('destroy', 'Delete')
                ->class('bx bx-trash me-1')
                ->target('_self')
                ->route('zone1.destroy', ['zone1' => 'id'])
                ->method('delete'),
    ...
    */
    ...
    public function actionRules(): array
    {
        return [
            ...
            //->hide(),
            Rule::button('edit')
                ->when(fn () => Auth::user()->cannot('zone1-edit'))
                ->hide(),

            Rule::button('delete')
                ->when(fn () => Auth::user()->cannot('zone1-delete'))
                ->hide(),

            Rule::button('pay')
                ->when(fn ($zone1) => ZoneMemberPay::where('member_id', $zone1->id)->where('model', 'zone1')->whereMonth('date', now()->month)->whereYear('date', now()->year)->exists())
                ->hide(),
            Rule::button('paid')
                ->when(fn ($zone1) => !ZoneMemberPay::where('member_id', $zone1->id)->where('model', 'zone1')->whereMonth('date', now()->month)->whereYear('date', now()->year)->exists())
                ->hide()
