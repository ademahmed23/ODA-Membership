...
final class Zone15Table extends PowerGridComponent
{
    use ActionButton;
    ...
    */
     ...
     *
     * @return Builder<\App\Models\Zone15>
     */
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('first_name_lower', function (Zone15 $model) {
    ...
    */
    ...
    /**
     * PowerGrid Zone15 Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::make('edit', 'Edit')
                ...
                ->target('_self')
                ->route('zone15.edit', ['zone15' => 'id']),

            Button::make('destroy', 'Delete')
                ->class('bx bx-trash me-1')
                ->target('_self')
                ->route('zone15.destroy', ['zone15' => 'id'])
                ->method('delete'),
    ...
    */
    ...
    public function actionRules(): array
    {
        return [
            ...
            //->hide(),
            Rule::button('edit')
                ->when(fn () => Auth::user()->cannot('zone15-edit'))
                ->hide(),

            Rule::button('delete')
                ->when(fn () => Auth::user()->cannot('zone15-delete'))
                ->hide(),
            Rule::button('pay')
                ->when(fn ($zone15) => ZoneMemberPay::where('member_id', $zone15->id)->where('model', 'zone15')->whereMonth('date', now()->month)->whereYear('date', now()->year)->exists())
                ->hide(),
            Rule::button('paid')
                ->when(fn ($zone15) => !ZoneMemberPay::where('member_id', $zone15->id)->where(
                    'model',
                    'zone15'
                )->whereMonth(
                    'date',
