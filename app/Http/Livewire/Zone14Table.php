...
final class Zone14Table extends PowerGridComponent
{
    use ActionButton;
    ...
    */
     ...
     *
     * @return Builder<\App\Models\Zone14>
     */
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('first_name_lower', function (Zone14 $model) {
    ...
    */
    ...
    /**
     * PowerGrid Zone14 Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::make('edit', 'Edit')
                ...
                ->target('_self')
                ->route('zone14.edit', ['zone14' => 'id']),

            Button::make('destroy', 'Delete')
                ->class('bx bx-trash me-1')
                ->target('_self')
                ->route('zone14.destroy', ['zone14' => 'id'])
                ->method('delete'),
    ...
    */
    ...
    public function actionRules(): array
    {
        return [
            ...
            //->hide(),
            Rule::button('edit')
                ->when(fn () => Auth::user()->cannot('zone14-edit'))
                ->hide(),

            Rule::button('delete')
                ->when(fn () => Auth::user()->cannot('zone14-delete'))
                ->hide(),
            Rule::button('pay')
                ->when(fn ($zone14) => ZoneMemberPay::where('member_id', $zone14->id)->where('model', 'zone14')->whereMonth('date', now()->month)->whereYear('date', now()->year)->exists())
                ->hide(),
            Rule::button('paid')
                ->when(fn ($zone14) => !ZoneMemberPay::where('member_id', $zone14->id)->where(
                    'model',
                    'zone14'
                )->whereMonth(
                    'date',
