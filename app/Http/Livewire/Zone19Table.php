...
final class Zone19Table extends PowerGridComponent
{
    use ActionButton;
    ...
    */
     ...
     *
     * @return Builder<\App\Models\Zone19>
     */
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('first_name_lower', function (Zone19 $model) {
    ...
    */
    ...
    /**
     * PowerGrid Zone19 Action Buttons.
     *
    ...
    public function actions(): array
    {
        return [
            Button::make('edit', 'Edit')
                ...
                ->target('_self')
                ->route('zone19.edit', ['zone19' => 'id']),

            Button::make('destroy', 'Delete')
                ->class('bx bx-trash me-1')
                ->target('_self')
                ->route('zone19.destroy', ['zone19' => 'id'])
                ->method('delete'),
    ...
    */
    ...
    public function actionRules(): array
    {
        return [
            ...
            //->hide(),
            Rule::button('edit')
                ->when(fn () => Auth::user()->cannot('zone19-edit'))
                ->hide(),

            Rule::button('delete')
                ->when(fn () => Auth::user()->cannot('zone19-delete'))
                ->hide(),
            Rule::button('pay')
                ->when(fn ($zone19) => ZoneMemberPay::where('member_id', $zone19->id)->where('model', 'zone19')->whereMonth('date', now()->month)->whereYear('date', now()->year)->exists())
                ->hide(),
            Rule::button('paid')
                ->when(fn ($zone19) => !ZoneMemberPay::where('member_id', $zone19->id)->where(
                    'model',
                    'zone19'
                )->whereMonth(
                    'date',
